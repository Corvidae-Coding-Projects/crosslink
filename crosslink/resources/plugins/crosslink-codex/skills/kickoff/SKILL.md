---
name: kickoff
description: Use to launch a background AI coding agent in tmux (or docker/podman) to implement a feature end-to-end. Delegates provider resolution, worktree creation, prompt building, and session launch to `crosslink kickoff run`, then prints how to attach (`tmux attach -t <session>`) and check status (`/check`). Trigger when the user says "kick off an agent for X", "/kickoff …", or asks for a background agent / sub-agent / worker to do a feature.
---
...
- `--model <model>`: provider model or semantic tier (`default`, `standard`, `advanced`). Default: `standard`.
...
1. **Validate prerequisites**: Run `crosslink doctor` and confirm `tmux` plus the configured provider CLI are available for local mode. If `--verify ci` or `--verify thorough`, check that `gh` is available. If missing, report the provider-specific installation or login action and stop.
