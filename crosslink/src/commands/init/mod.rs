...
// Embed canonical agent skills — auto-generated by build.rs.
...
/// Verify every repository-local Codex hook input against the init manifest.
/// This is the only gate allowed to enable Codex's one-shot hook-trust bypass.
pub(crate) fn codex_hook_trust_ready(project_root: &Path) -> Result<bool> {
    ...
    // The bypass applies to the entire enabled hook file. Never auto-trust a
    // file containing user/untracked top-level settings or hook groups, even
    // when Crosslink's own projected groups still match the manifest.
...
fn managed_files(
    ...
    // Canonical skills are rendered into each selected provider discovery root.
...
fn run_update(path: &Path, opts: &InitOpts<'_>) -> Result<()> {
    ...
    // Assets removed from Crosslink are retired only when their current bytes
    // still match the old manifest. Compute this before the dry-run gate so a
    // preview reports the same retire/preserve decisions that apply mode uses.
    ...
    // Retire assets that disappeared from Crosslink entirely. Provider assets
    // omitted only because of this invocation's selector are preserved.
...
mod tests {
    ...
    fn test_init_deploys_mcp_servers() {
        ...
        // Both provider-neutral server scripts must be deployed once.
        ...
        // .mcp.json must reference both MCP servers. If a server is
