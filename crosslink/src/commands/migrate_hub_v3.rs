...
use crate::hub_v3::{self, HubMeta, HubVersion, CHECKPOINT_REF, META_REF};
use crate::issue_file::{
    read_all_issue_files, read_all_milestone_files, read_comment_files, read_counters, IssueFile,
};
use crate::sync::SyncManager;
use crosslink::reconcile::publication::{
    CanonicalSemantic, HistoricalImporter, PreparedImport, PublicationOutcome,
    RepositoryReconciler, SourceEvidence,
};
use crosslink::reconcile::SharedStoreFormat;
    ...
    fn snapshot_source_refs(
        ...
        if matches!(source.format().shared_store, SharedStoreFormat::Absent) {
            ...
            let (name, oid) = if database.is_file() {
                (
                    "local/issues.db",
                    snapshot_file(repository, &database, "issues.db")?,
                )
            } else {
        ...
        if database.is_file() && has_complete_v3_source(source) {
            ...
            let local = build_genesis_from_database(&database, &self.agent_id)?;
...
fn merge_file_source_with_current_v3(
    ...
    let base_checkpoint = meta
        .genesis_checkpoint_commit
        .clone()
        .ok_or_else(|| anyhow::anyhow!("mixed v3 meta has no genesis checkpoint"))?;
    ...
    let base_state = compaction::reduce(&base_source)
...
pub fn hub_v3(
    ...
    run_reconciliation(crosslink_dir, &cache_dir, &remote, &hub_lock)?;
    if finalize {
        println!(
...
fn run_reconciliation(
    ...
    let format = crosslink::reconcile::check_repository(crosslink_dir).format;
    let local_source = matches!(&format.shared_store, SharedStoreFormat::Absent);
    ...
    let should_hydrate =
        !local_source && !matches!(&outcome, PublicationOutcome::ReadyCurrent { .. });
    match &outcome {
...
fn build_genesis_from_database(path: &Path, agent_id: &str) -> Result<CheckpointState> {
    ...
    let source_issues = database.list_issues(Some("all"), None, None)?;
    let source_milestones = database.list_milestones(Some("all"))?;
...
fn seed_v3_targets(
    ...
    for agent_id in agents {
        ...
        let oid = if current_bytes.as_deref() == Some(merged.as_slice()) {
...
mod tests {
    use super::*;
    use crate::hub_v3::{agent_ref_name, read_hub_meta};
    ...
    #[test]
    fn absent_repository_publishes_empty_verified_generation() {
        ...
        assert!(remote_rev(cache_dir, GENERATION_POINTER).is_some());
    ...
    fn fresh_clone_discovers_and_imports_remote_hidden_v3() {
        ...
        let sync = SyncManager::new(&crosslink_dir).unwrap();
        sync.init_cache_for_reconciliation().unwrap();
        let format = crosslink::reconcile::check_repository(&crosslink_dir).format;
        assert!(matches!(
            format.shared_store,
    ...
    fn late_remote_v2_branch_is_discovered_after_empty_generation() {
        ...
        hub_v3(&crosslink_dir, false, false, false, false).unwrap();
        assert_eq!(rev(cache_dir, V2_HUB_BRANCH), Some(late_tip.clone()));
        assert_ne!(
        ...
        hub_v3(&crosslink_dir, false, false, false, false).unwrap();
        assert_eq!(rev(cache_dir, V2_HUB_BRANCH), Some(next_tip));
        assert_ne!(
            remote_rev(cache_dir, GENERATION_POINTER),
    ...
    fn legacy_locks_source_publishes_and_preserves_archive() {
        ...
        hub_v3(&crosslink_dir, false, false, false, false).unwrap();
        assert_eq!(
            rev(&cache_dir, "refs/heads/crosslink/locks"),
            Some(legacy_tip)
        );
        let archives = Command::new("git")
            .current_dir(&cache_dir)
    ...
    fn mixed_partial_v3_is_rebuilt_from_pinned_v2_without_deletion() {
        ...
        hub_v3(&crosslink_dir, false, false, false, false).unwrap();
        assert_eq!(rev(&cache_dir, V2_HUB_BRANCH), Some(v2_tip));
    ...
    fn tracked_dirty_v2_edit_survives_remote_advance_and_reconciliation() {
        ...
        let remote_tip = rev(&source_cache, V2_HUB_BRANCH).unwrap();
        hub_v3(&crosslink_dir, false, false, false, false).unwrap();
        assert_eq!(rev(&cache, V2_HUB_BRANCH), Some(base.clone()));
    ...
    fn dormant_dirty_clone_imports_remote_only_v2_resurrection() {
        ...
        dormant_sync.init_cache_for_reconciliation().unwrap();
        ...
        assert_ne!(second_generation, first_generation);
        assert_eq!(rev(&dormant_cache, V2_HUB_BRANCH), Some(base));
    ...
    fn production_importer_fallback_two_clone_race_has_one_verified_adopter() {
        ...
        source_sync.init_cache_for_reconciliation().unwrap();
        second_sync.init_cache_for_reconciliation().unwrap();
        ...
        let source_format = crosslink::reconcile::check_repository(&source_crosslink).format;
        let second_format = crosslink::reconcile::check_repository(&second_crosslink).format;
