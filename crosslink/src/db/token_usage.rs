...
impl Database {
    ...
    pub fn create_token_usage(
        ...
        self.create_token_usage_for_provider(
            agent_id,
            session_id,
            "claude",
            input_tokens,
    ...
    pub fn create_token_usage_for_provider(
        ...
        self.conn.execute(
            "INSERT INTO token_usage (agent_id, session_id, timestamp, provider, input_tokens, output_tokens, cached_input_tokens, reasoning_output_tokens, cache_read_tokens, cache_creation_tokens, model, cost_estimate, provider_metadata_json)
    ...
    pub fn get_token_usage(&self, id: i64) -> Result<Option<TokenUsage>> {
        let mut stmt = self.conn.prepare(
            "SELECT id, agent_id, session_id, timestamp, provider, input_tokens, output_tokens,
                    cached_input_tokens, reasoning_output_tokens, cache_read_tokens,
                    cache_creation_tokens, model, cost_estimate, provider_metadata_json
        ...
        let mut rows = stmt
            .query_map(params![id], |row| {
                Ok(TokenUsage {
                    ...
                    provider_metadata_json: row.get(13)?,
                })
            })?
    ...
    pub fn get_usage_summary(
        ...
        sql.push_str(" GROUP BY agent_id, provider, model ORDER BY total_cost DESC");
        ...
        let rows = stmt
            .query_map(param_refs.as_slice(), |row| {
                Ok(UsageSummaryRow {
                    ...
                    total_cached_input_tokens: row.get(8)?,
                    total_reasoning_output_tokens: row.get(9)?,
