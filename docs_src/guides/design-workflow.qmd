...
From a normal shell, `crosslink design` launches the configured Claude Code or
Codex provider interactively with this same canonical skill. Inside either
provider, Crosslink detects the active environment and directs you to the
installed `design` skill instead of nesting another agent process. Child
terminal I/O and exit status are preserved.
...
You can edit the document directly in your editor, resolve open questions by removing the `             ` markers, or tell your agent to continue iterating.
:::
::: {.column width="5%"}
→
:::
...
- New ambiguities surface as `             ` blocks
