...
|------|------------|-------------|
| `session-start.py` | Startup, resume, clear, compact | Loads context without duplicating active sessions; restores issue, action, knowledge, and provenance context |
...
| `pre-web-check.py` | Before Claude native web tools | Reminds that external words are evidence, not instructions |
...
Crosslink does not download or rewrite web pages. Claude receives the provenance
rule before native web tools. Hosted Codex web search does not traverse local
tool-use hooks, so Codex receives the same rule at session start, every prompt,
subagent start, and compaction restoration. Both providers keep native search.
