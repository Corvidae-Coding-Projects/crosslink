...
Changes take effect on the next matching hook event.
...
## Refreshing hook assets

Tracking enforcement comes from `tracking_mode` and the hook implementation.
The prompt hook still resolves the matching tracking Markdown; the bundled files
are currently zero bytes. To refresh the managed assets:
