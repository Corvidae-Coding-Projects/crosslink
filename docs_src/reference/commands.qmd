...
| `crosslink doctor [--json]` | Provider, login, integration, plugin, MCP, hook-trust, and container credential diagnostics |
...
| `crosslink kickoff run <desc> --timeout 2h --model standard` | With timeout and provider-aware model tier |
...
| `crosslink container auth login --provider claude|codex` | Interactive normal-account login into a provider-scoped volume |
| `crosslink container auth status --provider claude|codex` | Redacted login readiness check |
| `crosslink container auth refresh --provider claude|codex` | Refresh the normal account session |
| `crosslink container auth logout --provider claude|codex` | Confirm and remove the credential volume (`--force` for non-interactive use) |
...
| Command | Description |
|---------|-------------|
| `crosslink context measure` | Measure installed hook, skill, reference, and compatibility assets |
...
| Command | Description |
|---------|-------------|
| `crosslink init` | Initialize Crosslink and both Claude/Codex integrations |
| `crosslink init --agent-integration claude|codex|both` | Select installed integration assets without changing `agent.provider` |
| `crosslink init --update [--dry-run]` | Manifest-safe managed asset upgrade/preview |
