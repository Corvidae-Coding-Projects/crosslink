...
```json
{
  ...
  "agent": {
    "provider": "claude",
    "providers": {
      "claude": {"standard_model": "sonnet", "advanced_model": "opus"},
      "codex": {"standard_model": null, "advanced_model": null, "sandbox": "workspace-write", "approval": "auto-review"}
    }
  },
...
Selected runtime protocol: `"claude"`, `"codex"`, or `"custom"`. Default:
`"claude"` for backward compatibility. This is independent from
`crosslink init --agent-integration`, which only chooses repository assets.
Local override wins over shared config.

### `agent.binary`

Optional executable-path override for the selected provider. It never changes
provider semantics. `custom` requires a non-empty value. Legacy configurations
without `agent.provider` infer the provider from `claude`, `codex`, or another
binary name.

### `agent.providers`

Per-provider default, standard, and advanced model mappings plus execution
defaults. A null Codex model omits `--model` and uses the signed-in account's
current default. Codex host sandbox defaults to `workspace-write` with automatic
review so Git metadata writes can be approved without disabling the sandbox;
container execution may use danger-full-access only inside external isolation.
...
Number of prompt events between prompt-hook reminders. Default: `3`.
...
Explicit absolute path to the `crosslink` binary used by the Python hook scripts when shelling out. String. Default: unset (hooks fall back to `$PATH`, then common install locations like `~/.cargo/bin/crosslink`). Set this when `crosslink` lives somewhere unusual or when multiple installs need disambiguating -- e.g. `crosslink config set crosslink_binary /opt/crosslink/bin/crosslink`.

### `context_budget_chars`

Estimated character budget used by the prompt hook's context counter. Integer.
The default is `1000000`; a non-positive value disables the budget check.

### `kickoff.allowed_tools`

Extra tool patterns appended to the kickoff policy. String array. Default: `[]`.
Claude maps these to `allowedTools`; other providers reject or map only options
they can preserve.
...
Optional sandbox-wrapper command applied to local kickoff launches. String.
The structured selected-provider invocation is wrapped only at the tmux shell
boundary; the value must be a binary on `$PATH` accepting a trailing command.
...
### `sentinel.default_agent.model_tier`

Semantic model tier or explicit provider model used for sentinel's first attempt.
Default: `"standard"`; it resolves through the selected provider.
The legacy key `model` remains accepted as an alias.
...
Enable automatic model escalation on first-attempt failure. Bool. Default: `true`. When `true`, sentinel retries failed dispatches with `sentinel.escalation.model_tier` after the cooldown.

### `sentinel.escalation.model_tier`

Semantic tier or explicit model for escalated retries. Default: `"advanced"`.
The legacy key `model` remains accepted as an alias.
