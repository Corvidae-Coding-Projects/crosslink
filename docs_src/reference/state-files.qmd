...
crosslink config set --local crosslink_binary /opt/crosslink/bin/crosslink
```
...
```json
{
  ...
  "description": "Driver agent for Corvidae-Coding-Projects/crosslink",
  "role": "driver",
...
| `rules/`, `rules.local/` | Rule-loader inputs. Bundled files are zero bytes; nonempty local overrides remain wired. See [Rules reference](rules.qmd). |
